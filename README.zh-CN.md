...
无法被检查的规范性陈述是编写错误，而不是一种更柔性的真相。原则见 [docs/VISION.md](docs/VISION.md)，七个子系统见 [docs/DESIGN.md](docs/DESIGN.md)，v0 语法见 [docs/SYNTAX.md](docs/SYNTAX.md)。

## 文档

[文档 Wiki](docs/wiki/app/content/zh/index.md) 分为三层：**概念**（为什么可判定规范是一场范式转变）、**机制**（循环如何闭合）与**指南**（如何投入使用）。从[快速开始](docs/wiki/app/content/zh/guides/01-quickstart.md)开始，或阅读 [Urtext 与规范驱动开发](docs/wiki/app/content/zh/concepts/04-vs-spec-driven-dev.md)，了解它与 Spec Kit 及同类工具的区别。

## 状态

v0 闭环，已自举：Urtext 在 `specs/urtext/` 中描述自己的核心行为，而 `urtext verify` 证明这一点——子句与清单解析器、不可变修订注册表、oracle runner（test/cmd/diff-scope/manual）、只追加证据，以及通过率加人工占比报告。下一里程碑：子句链接器（影响分析）和子句↔代码↔证据映射。
