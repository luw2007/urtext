...
需求提供了互补的第二个方向。`urtext impact <spec-path>#FR<n>` 先选择活跃 `req:`
唯一解析到该 FR 的子句；这些按 key 排序的 direct 子句成为种子，再沿同一 `refs`
反向闭包遍历并投影 checklist 任务。CLI 用单一列表和 `[direct]` / `[transitive]`
标记，因此直接守卫包含在受影响闭包中，同时角色仍然清楚。歧义或悬空绑定绝不算
守卫——这与 status 计算未覆盖需求使用同一真值。目标 FR 未声明或已 tombstone 时
Exit 1；FR 已声明但没有守卫时可见但不报错。

## 陈旧传播会让证据(evidence)失效

图不只是用于查询 — 它还驱动失效。当一个子句的 `text_hash` 改变（其标题或
正文文本 — 不是锚点元数据，所以 `oracle`/`risk`/`refs` 编辑不会触发它）时，
链接器会遍历反向闭包，并把每个依赖子句标记为 `stale`。每个陈旧子句的
现有证据会在同一条 UPDATE 中写入 `invalidated_at` 与
`invalidation_source` 这一枚逻辑作废戳。

source 是真实变更的 clause 或 FR key。直接命中 FR 的子句归因到该 FR；若该
子句文本也同时变更，它的 reverse-ref 下游仍归因到该子句，因为那才是使下游
失效的因果路径。首个印章不可改写，历史 NULL source 保持未知且绝不回填；
证据永不删除，只会作废直到重新验证。
